...
function StopSubmittingProofModal({cohort, major, levelYear, handleHiddenStopSubmittingProofModal}) {
    ...
    const handleStopSubmittingProof = () => {
        if((progressPercentage || progressPercentageInputRef.current.readOnly) && (score || scoreInputRef.current.readOnly)) {
            dispatch(stopSubmittingProof({
            ...
            <div className="stop_submitting_proof_modal">
                ...
                <div className="stop_submitting_proof_body">
                    <p className="stop_submitting_proof_body_content">
                        {`Bạn muốn kết thúc hoạt động nộp minh chứng năm ${levelYear} khóa ${cohort} của chuyên ngành ${capitalizeFirstLetter(major)}`}
                    </p>
                    ...
                    <ul className="stop_submitting_proof_body_notify">
                        ...
                        <li>{`Bạn phải tạo các chỉ tiêu mới của năm ${levelYear + 1} để sinh viên tiếp tục nộp minh chứng.`}</li>   
                        <li>{`Những sinh viên năm ${levelYear} khóa ${cohort} không đạt yêu cầu sẽ bị khóa tài khoản.`}</li>    
                        <li>{`Những sinh viên đạt yêu cầu sẽ được phép tiếp tục nộp minh chứng năm ${levelYear + 1}.`}</li>      
                    </ul>
                ...
                <div className="stop_submitting_proof_footer">
                    ...
                    <button 
                        className={`btn_accept ${confirmValue.trim().toLowerCase() === "tôi đồng ý" ? "active" : "inactive"}`} 
