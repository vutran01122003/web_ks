...
export const addTable = ({pageId, tables}) => async (dispatch) => {
    try {
        ...
        dispatch({
            type: GLOBALTYPES.ALERT,
            payload: {
                error: error?.response?.data.msg || "Thêm chỉ tiêu thất bại"
            }
        })
    }
}

export const getTables  = ({subPageName}) => async (dispatch) => {
    try {
        ...
        dispatch({
            type: GLOBALTYPES.ALERT,
            payload: {
                error: error?.response?.data.msg || "Lấy dữ liệu chỉ tiêu thất bại"
            }
        })
    }
}

export const removeTable = ({pageId, tableId}) => async (dispatch) => {
    try {
        dispatch({
            type: GLOBALTYPES.ALERT,
            payload: {
        ...
        dispatch({
            type: GLOBALTYPES.ALERT,
            payload: {
                error: error?.response?.data.msg || "Xóa chỉ tiêu thất bại"
            }
        })
    }
}