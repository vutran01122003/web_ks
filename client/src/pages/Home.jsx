...
const Home = ({ auth }) => {
    ...
    useEffect(() => {
        if(progress.goalsInfoData[levelYear]) {
            ...
            setChartData(progress.goalsInfoData[levelYear].map((elemProgress) => {
        ...
        <div className="pageHome ">
            ...
            <div className="container__top transform__animation--top">
                ...
                <>
                    ...
                    { auth?.user.roles.includes('0002') && <LayoutChart chartData={chartData} auth={auth} setLevelYear={setLevelYear}/>}
                </>
            </div>

            <div>
                { auth?.user.roles.includes('0002') && <GoalsInfo levelYear={levelYear} goalsInfo={goalsInfo} /> }
            </div>
        </div>
    )
}
